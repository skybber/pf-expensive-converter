...
public class BeanManagerImpl implements BeanManager, Referenceable
{
    ...
    private HashMap<Type, List<Bean<?>>> deploymentBeansFastLookup = new HashMap<>();
    ...
    {
    	if (newBean.getBeanClass() == javax.enterprise.inject.spi.BeanManager.class)
    	{
        ...
        if(newBean instanceof AbstractOwbBean)
        {
            ...
            for (Type beanType: newBean.getTypes())
            {
            	deploymentBeansFastLookup.computeIfAbsent(beanType, t -> new ArrayList<>())
            		.add(newBean);
            }
        }
        else
    ...
    public List<Bean<?>> getBeanFastLookup(Type beanType)
    {
    	List<Bean<?>> beanList = deploymentBeansFastLookup.get(beanType);
    	if (beanList != null)
    	{
    		return beanList.stream()
    			.filter(t->((OwbBean) t).isEnabled())
    ...
    public void clear()
    {
        ...
        deploymentBeansFastLookup.clear();
        errorStack.clear();
